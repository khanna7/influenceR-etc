...
double get_next_state_graph(graph_t *graph, int n, int *gen, int k, double p, int *ua, int *va, int round)
{
	int argmin[n];
	int s[k], t[n-k];
	regen(gen, s, t, n, k);
	...
	if (distance == NULL) {
		distance = (double *) malloc(n*k*sizeof(double));
		if (!distance) {
			printf("error with malloc!");
			exit(1);
		}
		BFS_multiple(graph, s, k, distance);
	}
	...
	for (int i = 0; i < k; i++) {
		...
		// copy
		for (int a = 0; a < n; a++)
			new_distance[X(j, a, n)] = distance[X(i, a, n)];
		j++;
	}
	if (j != k) {
		BFS_single(graph, v, &new_distance[j*n]);
